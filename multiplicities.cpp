...
//#include <string>
{
    ...
    // and Multiplicities! )
    double Egammamin = 0.450;    //diskusjon med Cec: i alle fall 400?! 300 er "original" threshold
    double Egammamax = 12.;
    double IntegralLowerLimit = 0.140;

    //================== ====== ==================
    // Neutron/gamma correction factor (~20% of all counts are neutrons. Correct for this. Ask Sunniva or Fabio)
    // double neutronCorrectionFactor = 0.78;
    double neutronCorrectionFactor = 1.0;   //max
    // double neutronCorrectionFactor = 0.84;   //min
    double oneMinusNeutronCorrectionFactor = 1.0 - neutronCorrectionFactor;
    ...
    // double neutronCorrectionFunctionScaling = 2.0 * oneMinusNeutronCorrectionFactor * 1.072 * 1e6 / exp(-2.0*Egammamin/1000.); // divided by 1000 because of units
    double neutronCorrectionFunctionScaling = 2.0 * oneMinusNeutronCorrectionFactor * 1.072 * 1e6; // We numerically find the scaling further down
	...
	int nIntervals = 7;
	// Region of Interest
	double Emin = 4.5; // Excitation energy
    double Emax = 8.8;
    ...
    double nGammaCoinc[nIntervals],nGammaCoincErr[nIntervals],
...
///////////////////////////////////////////
    ...
    TH1D *h1 = (TH1D*)SortingFile.Get("h_ex_fiss");
    // TH1I *h1cp = h1-> Clone();
    TH2D *h2 = (TH2D*)SortingFile.Get("m_alfna_fiss_neutron_corrected"); // denne må forandres ti den neutron corrected
    // // TH2F *h3a = h2-> Clone();
    // cout << "matrix:" << matrix->GetXaxis()->GetXmin() << endl;
    // cout << "test:" << h1->GetXaxis()->GetXmin() << endl;
    // cout << "test:" << h2->GetXaxis()->GetXmin() << endl;

    double xMin = h1->GetXaxis()->GetXmin();
    double xMax = h1->GetXaxis()->GetXmax();
    xMin /= 1000; // keV to MeV
    xMax /= 1000; // keV to MeV
    ...
    xMin /= 1000; // keV to MeV
    xMax /= 1000; // keV to MeV
    yMin /= 1000; // keV to MeV
    yMax /= 1000; // keV to MeV
    h2.GetXaxis()->SetLimits(xMin,xMax);
    h2.GetYaxis()->SetLimits(yMin,yMax);

    // cout << "testh2:" << h2->GetXaxis()->GetXmin() << endl;
    // cout << "testma:" << matrix->GetXaxis()->GetXmin() << endl;
    ...
    TH1D *hdiv=new TH1D("hdiv","Multiplicity (raw Spectrum)",h1->GetSize(),
                        h1->GetXaxis()->GetXmin(),
                        h1->GetXaxis()->GetXmax() );
    // TH1I *hdiv = h1-> Clone();
    // hdiv->Divide(h3,h1,1.,1.,"");

    TH1D *hdiv_unf=new TH1D("hdiv_unf","Multiplicity (unfolded spectrum)",h1->GetSize(),
                        h1->GetXaxis()->GetXmin(),
                        h1->GetXaxis()->GetXmax() );
    ...
    double xMin = m_alfna_fiss->GetXaxis()->GetXmin();
    double xMax = m_alfna_fiss->GetXaxis()->GetXmax();
    double yMin = m_alfna_fiss->GetYaxis()->GetXmin();
    double yMax = m_alfna_fiss->GetYaxis()->GetXmax();
    xMin /= 1000; // keV to MeV
    xMax /= 1000; // keV to MeV
    yMin /= 1000; // keV to MeV
    yMax /= 1000; // keV to MeV
    m_alfna_fiss.GetXaxis()->SetLimits(xMin,xMax);
    m_alfna_fiss.GetYaxis()->SetLimits(yMin,yMax);
    ...
    int i;
    double xmin[nIntervals], xmax[nIntervals];
    double xinterval;
    ...
    for (i=0; i<nIntervals; i++) {
        ...
        sstm.str(std::string());
        sstm << histname_base << " Emin " << xmin[i] << "MeV Emax "
             << xmax[i] <<"MeV";
        ...
        ///////////////////////////////////////////////
        // Rebin with bins xx width
        double binSize = 0.090; // originallt 8 keV per bin

        xMin = h4[i]->GetXaxis()->GetXmin();
        xMax = h4[i]->GetXaxis()->GetXmax();

        int numberOfBins = (xMax-xMin)/binSize;
        double lowestBin = xMin;
        // double lowestBin = 0;
        ...
        for(int binIndex=0; binIndex<=numberOfBins; binIndex++) {
            ...
            // cout << "lowerLimits" << binLowerLimits[binIndex] << endl;
        ...
        // double ta=nGammaCoinc[i];
        // double tb=nFissions[i];
        // cout << "nGammaCoinc: " << ta << endl;
        // cout << "nFissions: " << tb << endl;

        // double multiplicityErr[i] = multiplicity[i]*sqrt((1/ta)+(1/tb));
        ...
        binmin = h4[i]->FindBin(IntegralLowerLimit);
        binmax = numberOfBins;
        h4[i]->GetXaxis()->SetRangeUser(IntegralLowerLimit,Egammamax); // set Range to get correct values for e.g.GetMean
        ...
        ***********************************/
        ...
        // double binValue = 0;
        // double binErr = 0;
        // h4_err[i]->GetXaxis()->SetRangeUser(IntegralLowerLimit,Egammamax);
        ...
        for(int binIndex=0; binIndex < EgammaminBin; binIndex++) {
            if(h4[i]->GetBinCenter(binIndex) >= 0) {
               ...
               binValue = 1./3.*(  h4[i]->GetBinContent(EgammaminBin)
                                 + h4[i]->GetBinContent(EgammaminBin+1)
                                 + h4[i]->GetBinContent(EgammaminBin+2) );
               h4_err[i]->SetBinContent(binIndex, binValue);
               h4[i]->SetBinContent(binIndex, binValue);
               binErr = 0;
               h4[i]->SetBinError(binIndex, binErr);
               // calculate the uncert. as the difference between the first extrapolated bin lower boundardy and the
               // last extrapolated bin upper boundary.
               // upper/lower boundary = value +- uncert.
               double lowBinlowerBound =  h4[i]->GetBinContent(EgammaminBin)   - h4[i]->GetBinError(EgammaminBin);
               double highBinhighBound =  h4[i]->GetBinContent(EgammaminBin+2) + h4[i]->GetBinError(EgammaminBin+2);
               binErr = highBinhighBound - lowBinlowerBound;
               binErr = sqrt(n_binChanged) * binErr;                              // n_binChanged is
    ...
    // double n_hex, n_alfna_py, n_multi, nsize, nlength, nvalue;
    ...
    h1.GetXaxis().SetTitle("Excitation energy E_{x} [MeV]");
    h1.GetYaxis().CenterTitle();
    h1.GetYaxis().SetTitleOffset(1.1);
    h1.GetXaxis().SetTitleOffset(1.4);
    h1.GetYaxis().SetTitle("Counts/MeV");
    h1.GetYaxis().SetTitleSize(0.05);
    h1.GetXaxis().SetTitleSize(0.05);
    h1.GetYaxis().SetLabelSize(0.06);
    h1.GetXaxis().SetLabelSize(0.06);
    h1.GetXaxis().SetRangeUser(2.,13.);
    h1.SetLineColor(kBlack);
    h1.SetLineWidth(2);
    h1.Scale(1/x_cal[1]);
    h1.GetYaxis().SetRangeUser(500,3e6);
    ...
    TLegend *leg = new TLegend(0.67,0.651742, 0.997126, 0.868644);
    leg.SetBorderSize(0);
    leg.SetFillStyle(0);
    ...
    TLine *line1 = new TLine(Emin,0,Emin,1e6);
    line1->SetLineStyle(2);
    line1->SetLineWidth(2);
    line1->Draw();

    TLine *line4 = new TLine(Emax,0,Emax,1e6);
    ...
    double Sn      = 6.534;
    double FissBar = 6.05;
    double FissBar2 = 5.15;
    TArrow *line2 = new TArrow(FissBar,200e3,FissBar,51e4,0.02,">");
    line2->SetLineStyle(1);
    line2->SetLineWidth(3);
    line2->Draw();

    TArrow *line22 = new TArrow(FissBar2,500e2,FissBar2,110e3,0.02,">");
    line22->SetLineStyle(1);
    line22->SetLineWidth(3);
    line22->Draw();

    TArrow *line3 = new TArrow(Sn,200e3,Sn,51e4,0.02,">");
    ...
    TLatex t;
    t.SetTextSize(0.05);
    t.DrawLatex(Sn,120e3,"S_{n}");
    t.DrawLatex(FissBar-0.350,120e3,"B_{F,a}");
    t.DrawLatex(FissBar2-0.300,30e3,"B_{F,b}");
    ...
    h4[0].GetXaxis().SetTitle("E_{#gamma} [MeV]");
...
//    h4[0].GetXaxis().SetLabelOffset(1.4);
    ...
    // h4[0].GetXaxis().SetRangeUser(0,15);
    h4[0].GetXaxis().SetRangeUser(0,9.8);
...
//     recalhist->Draw("same");
    ...
    TLegend *leg = new TLegend(0.61,0.563559  ,0.968391,0.891949);
    leg.SetBorderSize(0);
    leg.SetFillStyle(1);
    ...
    for (i=0; i<nIntervals; i++) {
        ...
        sstm.str(std::string());
        sstm << " E_{x} = " << std::setprecision(3) << xmin[i] << " - " << xmax[i] <<" MeV";
        string_result = sstm.str();
        histname_result = string_result.c_str();
...
/////// Histogram over the muliplicites //////////////////
    ...
    double THEORY_ex_energy[100];
    double THEORY_multiplicity[nArrays][100], THEORY_energy_av[nArrays][100], THEORY_energy_tot[nArrays][100];
...
///////////////////////////////////////////////////////////////
   ...
   Double_t h = 800;
    ...
    h_multi_1.GetXaxis().SetTitle("Excitation energy E_{x} [MeV]");
    ...
    h_multi_1.GetYaxis().SetLabelSize(0.07);
    h_multi_1.GetXaxis().SetLabelSize(0.07);
    double ymin = 5.1;
    h_multi_1.GetYaxis().SetRangeUser(ymin,10.7);
    ...
    theory_multiplicity_graph[i]->SetLineWidth(2.8);
        ...
        else if(i==3) {colour = colour3;
            theory_multiplicity_graph[i]->SetLineStyle(7);}
   ...
   float PlAlphaValue = 1;
   float PlShift = 0.04;
   TGraphErrors *OtherExp = new TGraphErrors(1);
   // OtherExp->SetFillColor(23);
   OtherExp->SetMarkerColor(1);
   OtherExp->SetMarkerStyle(23);
   OtherExp->SetMarkerSize(1.4);
   OtherExp->SetLineWidth(2);
   double y_value = 7.24;
   double y_err   = 0.7;
   //   (PointNumber , X , Y)
   OtherExp->SetPoint     (0,Sn-PlShift,y_value);
   OtherExp->SetPointError(0, 0, y_err);
   OtherExp->Draw("P");
    // theory_multiplicity_graph->SetMarkerColor(kBlue);


    TGraphErrors *Pleasanton = new TGraphErrors(1);
    Pleasanton->SetMarkerColor(1);
    Pleasanton->SetLineColor(1);
    Pleasanton->SetMarkerStyle(22);
    Pleasanton->SetMarkerSize(1.4);
    Pleasanton->SetLineWidth(2);
    double y_value = 6.88;
    double y_err   = 0.35;
    //   (PointNumber , X , Y)
    Pleasanton->SetPoint     (0,Sn+PlShift,y_value);
    Pleasanton->SetPointError(0, 0, y_err);
    Pleasanton->Draw("P");
...
////////////
    ...
    TLine *line12 = new TLine(FissBar2,ymin,FissBar2,1e5);
    // line1->SetLineStyle(2);
...
// TEST ONLY
   ...
   // TGraphErrors *gr = new TGraphErrors(8, meanEnergies, AverageEnergy, 0, 0);
	gr.GetYaxis().SetRangeUser(705/1000.,1.47);
	gr->SetTitle("Sunniva er hot");
	gr->GetYaxis().CenterTitle();

	gr->GetYaxis().SetTitle("Average #gamma-energy [MeV]");
   ...
   grup->Draw("same");
    ...
    theory_energy_av_graph[i]->SetLineWidth(2.8);
            ...
            theory_energy_av_graph[i]->SetLineStyle(7);}
...
c2.cd(1);
   ...
   // TGraph *gr = new TGraph(nIntervals, meanEnergies, TotalEnergy); //errors med hensyn på relative neutron contribution
	gr.GetYaxis().SetRangeUser(5.01,10.9);
	gr->SetTitle("Sunniva er hot");
	gr->GetYaxis().CenterTitle();
	gr->GetYaxis().SetTitle("Total #gamma-energy [MeV]");
	gr->GetYaxis().SetTitleSize(0.078);
	gr->GetYaxis().SetTitleOffset(0.73);
	gr.GetYaxis().SetLabelSize(0.08);
	gr.GetXaxis().SetLabelSize(0.08);
   ...
   grup->Draw("same");
    ...
    theory_energy_tot_graph[i]->SetLineWidth(2.8);
            ...
            theory_energy_tot_graph[i]->SetLineStyle(7);}
   ...
   OtherExp->Draw("P");
    ...
    TLegend *leg = new TLegend(0.171,0.556,0.31,0.972);
    leg.SetBorderSize(0);
    ...
    leg->SetTextSize(0.06);
    leg->AddEntry(h_multi_1,"Present exp.","ep");
    leg->AddEntry(theory_energy_tot_graph[0],"GEF J_{rms}=0", "l");
    leg->AddEntry(theory_energy_tot_graph[1],"GEF J_{rms}=8", "l");
    leg->AddEntry(theory_energy_tot_graph[2],"GEF J_{rms}=12","l");
    leg->AddEntry(theory_energy_tot_graph[3],"GEF J_{rms}(E#scale[0.7]{x})","l");
    leg->AddEntry(OtherExp,"Verbinski et al. (1973)","ep");
    leg->AddEntry(Pleasanton,"Pleasanton (1973)","ep");
    // leg->AddEntry(OtherExp,"Pleasonton (1973)","ep");
...
//    c1_1->SetTopMargin(0.09);
    ...
    matrix->GetXaxis()->SetTitle(" #gamma-ray energy E_{#gamma} (MeV)");
...
//    matrix->GetYaxis()->SetRangeUser(0.,12.3);
    matrix->GetXaxis()->SetRangeUser(0.,11.);
    matrix->GetYaxis()->SetRangeUser(0.,11.);
    matrix->GetZaxis()->SetRangeUser(1.,3.);
    matrix->Draw("col");
...
thefile.open("Theory_Christelle/GEF_gamma_spectrum_240Pu_E6.53MeV.txt");
int nheaderlines = 0;
...
// shift_filenames.resize(nPoints);
// shift_channels.resize(nPoints);
double energy1[nPoints], spectrum1[nPoints], spectrum1Err[nPoints];
for(i=0;i<nPoints;i++) {
	energy1[i]      = 0;
	spectrum1[i]    = 0;
	spectrum1Err[i] = 0;
...
// nPointsNew = nPoints / binSize
i = 0;
  while(thefile){
    thefile >> y;            // readfrist columns into "x"
    if(i<nPoints){
		energy1[binNumber]   = (binNumber+1/2.)*binSize/1.e3; // to get to keV
		spectrum1[binNumber] += y;
    	if (j == binSize) {
        spectrum1Err[binNumber] = sqrt(spectrum1[binNumber]);
        multiplier = 1e3/binSize/nEvents;
        spectrum1[binNumber] *= multiplier;
        spectrum1Err[binNumber] *= multiplier;
            binNumber++;
            j = 0;
            }
...
//     // TGraphAsymmErrors (Int_t n, const Double_t *x, const Double_t *y, const Double_t *exl=0, const Double_t *exh=0, const Double_t *eyl=0, const Double_t *eyh=0)
TGraphErrors *GEF = new TGraphErrors(nPoints,energy1,spectrum1,0,spectrum1Err);
...
// Verbinski and a spectrum at around S_n
    ...
    h4[nSelect].GetXaxis().SetTitle("E_{#gamma} [MeV]");
...
//    h2[0].GetXaxis().SetLabelOffset(1.4);
    ...
    h4[nSelect].GetXaxis().SetRangeUser(0,12.8);
    ...
    GEF->SetMarkerColor(kBlue);
    GEF->SetMarkerSize(1);
    GEF->SetLineColor(kBlue);
    ...
    // leg->AddEntry(h4[i],histname_result,"L")
    leg->AddEntry(Verbinski,"Verbinski","ep");
    leg->AddEntry(GEF,"GEF at E_{x} = 6.53 MeV","ep");
...
///////////////////////////////////
    //RATIO plot
    ///////////
    // TH1D *Ratio = dynamic_cast<TH1D*>( h4[i]->Clone() );
    int gammaBins = h4[0]->GetXaxis()->GetLast();
    double binErr, ratio, content1, content1Err, content2, content2Err;
    int n1 = 0;
    int n2 = 3;
    TH1D *h4_err_orig = dynamic_cast<TH1D*>( h4_err[n1]->Clone() );

    TCanvas *c6 = new TCanvas("c6","Ratio",900,600);
    ...
    h4_err[n1]->GetXaxis()->SetTitle("#E_{#gamma} [MeV]");
    h4_err[n1]->GetYaxis()->SetTitle("Ratio N[4.6MeV] / N[6.6MeV]");
    h4_err[n1]->GetXaxis()->SetRangeUser(0,11.9);
    h4_err[n1]->GetYaxis()->SetRangeUser(0,1.9);
    h4_err[n1]->Divide(h4_err[n2]);
    h4_err[n1]->Draw();

    c6->Print("ratio.pdf");
    h4_err[n1] = h4_err_orig; // undo scaling for writing results below.
...
cout << "MeV" << "\t" << "MeV" << "\t" << "MeV" << "\t" "MeV" << "\t" "MeV" << "\t" << "MeV"       << "\t" << "MeV"             << "\t" << "ph/fiss" << "\t" << "ph/fiss" << "\t" << "ph/fiss" << endl;
...
/////////////////////////////
// Saving the histograms to disk

ofstream myfile;

// h_cnt_per_ex
    myfile.open ("h7_cnt_per_ex.dat");

    double column[10];

    myfile << "Energy"  << '\t' << "d,pf"  << '\t' << "(d,pfg)" << endl;
    myfile << "MeV"  << '\t' << "cnt/MeV"  << '\t' << "cnt/MeV" << endl;

    for (i=0; i<h1->GetSize(); i++) {
    column[0] = h1->GetBinCenter(i);          //  x -value
    column[1] = h1->GetBinContent(i);         //  ^{239}Pu(d,pf)
    column[2] = h3_unf->GetBinContent(i);     //   ^{239}Pu(d,pfg)

    myfile << column[0]  << '\t' << column[1]  << '\t' << column[2] << endl;
    }

    myfile.close();


// All fission gamma spectra together
    myfile.open ("h8_all_fiss_spectra.dat");
    ...
    for (i=0; i<nIntervals; i++) {
        sstm.str(std::string());
        sstm << std::setprecision(3) << xmin[i] << "-" << xmax[i];
        string_result = sstm.str();
        histname_result = string_result.c_str();
        myfile << "\t" << histname_result << "\t" << "unc_stat" << "\t" << "unc_tot";
    }
    myfile  << endl;

        myfile << "Eg [MeV]";
    for (i=0; i<2*nIntervals; i++) {
        myfile << "\t" << "ph/fiss/MeV"  << "\t" << "ph/fiss/MeV";
    }
    myfile  << endl;


    // hist content
    for (i=0; i<h4[0]->GetSize(); i++) {
    column[0] = h4[0]-> GetBinCenter(i);
    myfile << column[0];

    for (int hist=0; hist<nIntervals; hist++) {
    column[1] = h4[hist]->GetBinContent(i);
    column[2] = h4[hist]->GetBinError(i);
    column[3] = h4_err[hist]->GetBinError(i);
    myfile << "\t" << column[1] << "\t" << column[2] << "\t" << column[3];
    }
    myfile << endl;
    }
    myfile.close();
...
// // Verbinski vs Sn
//     myfile.open ("h10_Verbinski_vs_Sn.dat");
...
//         string_result = sstm.str();
//         histname_result = string_result.c_str();
//         myfile << "\t" << histname_result << "\t" << "+-unc.";
//     }
//     myfile  << "\t" << <<"GEF" << "\t" << "unc" << "\t" << "Verbinski" << "unc" << endl;
